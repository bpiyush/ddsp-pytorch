...
def num_trainable_params(model, round=3):
    n_params = sum([p.numel() for p in model.parameters() if p.requires_grad])
    model_name = model.__class__.__name__
    print(f" [:::] Number of trainable parameters in {model_name}: {np.round(n_params / 1e6, round)}M")


def num_params(model, round=3):
    n_params = sum([p.numel() for p in model.parameters()])
    model_name = model.__class__.__name__
    print(f" [:::] Number of total parameters in {model_name}: {np.round(n_params / 1e6, round)}M")
