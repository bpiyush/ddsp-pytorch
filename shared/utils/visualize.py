...
def show_grid_of_images(
        ...
        ysuptitle=0.8, xlabels=None, sizealpha=0.7, show=True, row_labels=None,
    ):
...
def show_audio_and_image(audio_args, image_args, figsize=(6, 2)):
    audio = show_single_audio(**audio_args)
    
    # Show image
    img_out = widgets.Output()
    with img_out:
        fig, ax = plt.subplots(1, 1, figsize=figsize)
        ax.imshow(image_args["image"])
        ax.axis("off")
    image = VBox([Label("Image"), img_out])

    return HBox([audio, image])
